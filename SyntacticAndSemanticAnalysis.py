...
def ErrorParser(ErrorOfParser):
    if ErrorOfParser == 201:
        ...
        print("Error:", ErrorOfParser, "Ожидалось ';' после ключевого слова '|' или перенос строки до ключевого слова '|' или объявлен ещё один тип данных или другая ошибка")
        exit(0)
    elif ErrorOfParser == 203:
        print("Error: ", ErrorOfParser, "Ожидалось ';' или перенос строки")
        exit(0)
    elif ErrorOfParser == 201: #204
        print("Error:", ErrorOfParser, "ожидалось '}'")
        exit(0)
...
def ForCycle():
    ...
    if ID() or Numer() or Equals("true") or Equals("false") or Equals("not") or Equals("("):
        if ID() and not(Check()):
            ErrorParser(302)
        Expression()
    else:
...
def WhileCycle():
    ...
    if ID() or Numer() or Equals("true") or Equals("true") or Equals("not") or Equals("("):
        if ID() and not(Check()):
            ErrorParser(302)
        Expression()
    else:
...
def Tipisation():
    if Equals("/*"):
        Comment()
    if not(Equals("integer")) and not(Equals("real")) and not(Equals("boolean")):
        ErrorParser(207)
    Get_Lexem()
    return  True
...
def Ratio():
    if Equals("/*"):
        Comment()
    if not(Equals("< >") or Equals("=") or Equals("<") or Equals("<=") or Equals(">") or Equals(">=")):
        ErrorParser(219)
    Get_Lexem()
...
def Expression():
    ...
    if Numer() or ID() or Equals("true") or Equals("false") or Equals("not") or Equals("("):
    ...
    if Equals("< >") or Equals("=") or Equals("<") or Equals("<=>") or Equals(">") or Equals(">="):
        Ratio()
